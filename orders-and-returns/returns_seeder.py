...
def insert_into_returns(conn, returns_data):
    with conn.cursor() as cur:
        cur.executemany("""
            INSERT INTO returns (orderID, productName, returnDate, reason)
            VALUES (%s, %s, %s, %s);
        """, returns_data)
        conn.commit()
...
def insert_into_customer_feedback(conn, feedback_data):
    with conn.cursor() as cur:
        cur.executemany("""
            INSERT INTO customer_feedback (returnID, customerID, feedback, feedbackDate)
            VALUES (%s, %s, %s, %s);
        """, feedback_data)
        conn.commit()
...
def insert_into_return_processing(conn, processing_data):
    with conn.cursor() as cur:
        cur.executemany("""
            INSERT INTO return_processing (returnID, processingStep, status, updateDate)
            VALUES (%s, %s, %s, %s);
        """, processing_data)
        conn.commit()
...
def main():
    ...
    # Create and populate the tables in the 'returns' database
    ...
    conn.close()
    print(f"Inserted {num_rows} rows into the 'returns', 'customer_feedback', and 'return_processing' tables.")
